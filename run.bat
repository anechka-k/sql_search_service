	...
	if not %javaVersion% geq 1.7 (
		if not %javaVersion32% geq 1.7 (
			echo Java version is lower than 1.7, please install Java 1.7 or later
			goto endProcess
		) else (
...
:startJava	
        "%javaDir%"\bin\java.exe -cp sql_search_service.jar;lib\*;conf sqlsearch.Application
